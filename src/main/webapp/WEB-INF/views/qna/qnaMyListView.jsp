...
<c:set var="url" value="qmylist.do"></c:set>
...
<thead>
	<tr>
		...
		<th scope="col" class="th-date">답변</th>
	</tr>
	</thead>
<tbody>
	<c:forEach items="${requestScope.list}" var="q">
			<tr align="center">
				...
				<c:url var="qdt" value="/qdetail.do">
					<c:param name="user_id" value="${q.user_id}" />
				...
				<td>${q.user_id}</td>
				<td><c:if test="${q.inquiry_state == 0}">처리 중</c:if> 
				...
				<td>
					<c:if test="${q.inquiry_state == 1}">
					<c:url var="detail" value="/qmydetail.do">
							<c:param name="qna_no" value="${q.qna_no}" />
							<c:param name="qna_lev" value="${q.qna_lev}" />
							<c:param name="qna_ref" value="${q.qna_ref}" />
							<c:param name="user_id" value="${q.user_id}" />
					</c:url>
						<button class="btn2 btn-gray" onclick="javascript:location.href='${detail}';">답변보기</button>
					</c:if>
				</td>
			</tr>
		</c:forEach>
...
<!-- 페이징 처리 -->
<c:import url="/WEB-INF/views/common/qna_paging.jsp" />
<hr>
