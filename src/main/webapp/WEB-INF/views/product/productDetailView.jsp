				...
				<div class="col-lg-6 product-details pl-md-5 ftco-animate">
					...
					<p class="store">
						<span>매장 : </span> <select id="store_id">
							<option value="" selected>수령할 매장을 선택하세요</option>
                        ...
                        $("button.btn-cart")
                              .click(
                                    function() {
                                       ...
                                       // 로그인 여부 확인
                                       if (!"${sessionScope.loginUsers}") {
                                          var check = confirm("로그인 후 이용해주세요.");
                                          if (check) {
                                             ...
                                             .ajax({
                                                ...
                                                success : function(
                                                   ...
                                                   if (result
                                                      ...
                                                      if (check) {
                                                         location.href = "${pageContext.servletContext.contextPath}/cart.do?product_id=${product.product_id}&user_id=${loginUsers.user_id}&store_id="
                                                               + selectstore;
                                                      }
                                                   }
                                                },
                        ...
                        $("button#btn-pay")
                              .click(
                                    function() {
                                       ...
                                       var check = confirm("선택한 매장 : "
                                             + selectstorename
                                             + "\n결제를 진행하시겠습니까?");
                                       if (check) {
                                          location.href = "${pageContext.servletContext.contextPath}/orderinfo.do?product_id=${product.product_id}&product_name=${product.product_name}&product_image=${product.product_image}&user_id=${loginUsers.user_id}&order_qty="
                                                + $('#quantity')
                                                      .val()
                                                + "&store_id="
                                                + selectstore
